	...
	*/
	...
	while (fgets(line,MAXLINE,fp) != NULL) {
		if (isdigit(line[38]) == 1) {
			...
			if (isdigit(line[ppcol])) {
				nph++;
				substr(aux, line, ppcol+1, 8);
				nf = sscanf(aux, "%d %d %d", &pp_hour, &pp_min, &pp_sec);
				if (nf != 3) fprintf(stderr, "ERROR: invalid format for PP phase: **%s**\n%s", aux, line);
				...
				if (nph == 1)
					w_isc5(card[ncard], 0, year, month, "????", 0, ' ', ' ', ' ', ' ', az, dist,
					0, day, pp_hour, pp_min, (float)pp_sec, sec_acc, id, "PP", 999.9, isc_id, 999.9, ' ',
					' ', cmp, ' ');
				else
					w_isc6(card[ncard], 0, year, month, nph, day, pp_hour, pp_min, (float)pp_sec,
					sec_acc, id, "PP", 999.9, isc_id, 999.9, ' ', ' ', cmp, ' ');
				fprintf(fpout, "%s\n", card[ncard]);
				ncard++;
			}
